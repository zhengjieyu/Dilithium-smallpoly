...
void prepare_s1_table(uint64_t s_table[4*N], polyvecl *s1)
{
	uint32_t k,j;
    uint64_t temp , temp2;
	uint64_t mask_s = 0X4040404;
...

{
    ...
    for(i=0; i<N; i++)
	{
        ...
        for(j=0; j<L; j++)
		{
			z->vec[L-1-j].coeffs[i] = ((int32_t)(temp & 0xFF)-TAU*ETA);//(t mod M) - rU (mod q)
			temp >>= 8;//t = t/M
		}
...
void prepare_s2_table(uint64_t s2_table[4*N], polyveck *s2)
{
	uint32_t k,j;
    uint64_t temp , temp2;
    uint64_t mask_s = 0X4040404;
...
void evaluate_cs2(polyveck *z, const poly *c, const uint64_t s2_table[4*N])
{
    uint32_t i,j;
    ...
    for(i=0; i<N; i++)
	{
        ...
        for(j=0; j<K; j++)
		{
			z->vec[K-1-j].coeffs[i] = ((int32_t)(temp & 0xFF)-TAU*ETA);//(t mod M) - rU (mod q)
			temp >>= 8;//t = t/M
		}
	}
...
void prepare_t0_table(uint64_t t00_table[4*N], uint64_t t01_table[4*N], polyveck *t0)
{
	uint32_t k,j;
    uint64_t temp , temp2;
...
void prepare_t1_table(uint64_t t10_table[4*N], uint64_t t11_table[4*N], polyveck *t1)
{
	uint32_t k,j;
    uint64_t temp , temp2;
		...
		const uint64_t t10_table[4*N], const uint64_t t11_table[4*N])
{
    ...
    for(i=0; i<N; i++)
	{
        ...
        for(j=0; j<2; j++)
		{
			z->vec[1-j].coeffs[i] = ((int32_t)(temp & 0x1FFFF)-39936);//(t mod M) - rU (mod q)
			temp >>= 17;//t = t/M
        ...
        for(j=0; j<2; j++)
		{
			z->vec[3-j].coeffs[i] = ((int32_t)(temp & 0x1FFFF)-39936);//(t mod M) - rU (mod q)
			temp >>= 17;//t = t/M
		}
	}
}
